...
| **Eval consumer** | `src/pipeline/entry/evalQueueEntry.ts`, `wrangler-eval-consumer.jsonc` | Queue consumer: orientation ΓåÆ card extraction (OpenAI) ΓåÆ Scryfall ΓåÆ D1 ΓåÆ oriented images on R2 |
...
| **Redirect Worker** | `wrangler-redirect.jsonc` | Production only: `cubewizard.org` ΓåÆ `cube-wizard.com` |
...
**Eval pipeline** (two queue stages ΓÇö separate queue invocations; isolates may be reused by the platform but deck buffers are not retained across messages):

1. **Orient** (`runOrientTask` on `EVAL_QUEUE`): load staging image ΓåÆ orient (OpenAI) ΓåÆ upload oriented JPEG to `cubewizard-deck-images` ΓåÆ enqueue extract message.
2. **Extract** (`runExtractTask` on `EVAL_EXTRACT_QUEUE`): load oriented JPEG ΓåÆ extract card names (OpenAI) ΓåÆ upload WebP thumb ΓåÆ release RGBA ΓåÆ Scryfall ΓåÆ D1 ΓåÆ mark `processing_jobs` done.
...
**Eval queue concurrency:** `max_batch_size: 1` (one deck per invocation) and `max_concurrency` > 1 in `wrangler-eval-consumer.jsonc` so Cloudflare runs multiple consumer isolates when the queue has backlog (stg/prod: 4). Keep `CW_EVAL_MAX_CONSUMERS` equal to `max_concurrency` so Scryfall HTTP stays near ~10 req/s account-wide. **Local `wrangler dev` does not run multiple queue consumers in parallel** ([Cloudflare Queues local dev](https://developers.cloudflare.com/queues/configuration/local-development/)) ΓÇö you will only see one eval at a time locally even with `max_concurrency: 2`; deploy to staging/production for real parallelism.
...
| **Queues** | `*-local` / `*-local-dlq` (Miniflare only) | `cubewizard-eval-stg`, `cubewizard-hedron-stg`, ΓÇª | `cubewizard-eval-prod`, `cubewizard-hedron-prod`, ΓÇª |
...
Use **`npm run dev:all`** (or `npm run dev:terminals` on Windows to open it in a new window). Running `dev`, `dev:eval-consumer`, and `dev:hedron-consumer` in **separate** terminals does **not** share queues locallyΓÇöthe eval queue will not drain.
...
| `CW_EVAL_LOG_LEVEL` | No | `off` \| `low` \| `medium` \| `high` ΓÇö see [OpenAI log levels](#openai-log-levels-cw_eval_log_level) |
...
| `CW_EVAL_MEMORY_LOG` | No | `1` / `true` / `yes` ΓåÆ attach `heap_used_mb` / `rss_mb` to each `eval_consumer` log line (see [Memory debugging](#memory-debugging-eval-consumer)) |
...
Hosted eval also needs `R2_ACCESS_KEY_ID` and `R2_SECRET_ACCESS_KEY` (same R2 API token) via `wrangler secret put` ΓÇö see [Deploy](#deploy-cloudflare).
...
| `npm run golden:baseline` | Promote `scores/latest.json` ΓåÆ `scores/baseline.json` after confirmation (`--yes` to skip prompt) |
...
**Staging normalize (Phase 1):** Upload and Hedron staging use the **Cloudflare Images** Worker binding (`env.IMAGES`) to downscale photos to `CW_STAGING_MAX_IMAGE_SIDE` (default 3072 px) **before** R2 ΓÇö transform runs outside the isolate, avoiding full-resolution RGBA decode (~93 MB on 12 MP JPEGs). Each normalize is one Images transformation (see [Images pricing](https://developers.cloudflare.com/images/pricing/)); binding calls may require **Images Paid** on your account. Local `wrangler dev` uses a low-fidelity Images sim; set `"remote": true` on the `images` binding or use `wrangler dev --remote` for production parity.

| Approach | When to use |
|----------|-------------|
| **`CW_EVAL_MEMORY_LOG=1`** in `.dev.vars` or eval worker vars | Each **`eval_consumer`** JSON line includes `est_*` buffer MB (RGBA/JPEG sizes) plus `heap_used_mb` / `rss_mb` when `process.memoryUsage()` is real (`enable_nodejs_process_v2` on the eval worker). All-zero heap ΓåÆ `heap_probe_unavailable` and rely on `est_*`. |
...
| **Dashboard ΓåÆ Workers ΓåÆ eval consumer ΓåÆ Observability ΓåÆ Logs** | Query `eval_consumer` or `$metadata.trigger` / invocation errors; Metrics ΓåÆ Errors ΓåÆ **Exceeded Memory**. |
...
`est_rgba_mb` is the oriented frame at max side (W├ùH├ù4); `est_rgba_peak_mb` adds a rotation scratch during orient scoring. These are estimates, not isolate heap ΓÇö compare them to the 128 MiB limit when `heap_used_mb` is unavailable locally.
...
| **`main`** | Production. Updated when maintainers **promote** `staging` ΓåÆ `main`. Pushes deploy **production** ([`deploy-cloudflare-prod.yml`](.github/workflows/deploy-cloudflare-prod.yml)). |
...
| `/api/upload` | POST | Upload deck image ΓåÆ R2 ΓåÆ eval queue |
...
CubeWizard/
Γö£ΓöÇΓöÇ src/
Γöé   Γö£ΓöÇΓöÇ worker.js                 # Site Worker
Γöé   Γö£ΓöÇΓöÇ hedron-consumer.js        # Hedron queue consumer
Γöé   Γö£ΓöÇΓöÇ processingJobsD1.js       # Shared processing_jobs upsert
Γöé   ΓööΓöÇΓöÇ pipeline/                 # Eval pipeline (TypeScript, Vitest)
Γö£ΓöÇΓöÇ docs/                         # Static site (HTML/CSS/JS)
Γö£ΓöÇΓöÇ fixtures/pipeline/            # Task JSON schema + examples
Γö£ΓöÇΓöÇ vendor/jsquash-webp/          # Vendored WebP WASM
Γö£ΓöÇΓöÇ migrations/                   # D1 incremental SQL
Γö£ΓöÇΓöÇ schema.sql
Γö£ΓöÇΓöÇ wrangler.jsonc
Γö£ΓöÇΓöÇ wrangler-eval-consumer.jsonc
Γö£ΓöÇΓöÇ wrangler-hedron-consumer.jsonc
ΓööΓöÇΓöÇ wrangler-redirect.jsonc
```
