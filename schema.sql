...
-- Async eval / upload job status for /api/processing-decks (Worker reads processing_jobs in D1)
...
CREATE INDEX IF NOT EXISTS idx_processing_jobs_cube_status ON processing_jobs (cube_id, status);
CREATE INDEX IF NOT EXISTS idx_processing_jobs_submitted ON processing_jobs (cube_id, submitted_at);
