...
function hedronImageUrl(deckPath) {
  var p = String(deckPath || "").trim();
  if (!p) return "";
  return p.indexOf("http") === 0 ? p : HEDRON_ORIGIN + p;
}

function hedronR2Prefix(deckImageUuid) {
...
async function enqueueHedronDeckJobs(env, cubeId, jobs) {
  ...
  if (!q || typeof q.sendBatch !== "function") {
    ...
    return { ok: false, queued: 0, reason: "missing_queue" };
  }

  var batchSize = hedronEnvInt(env, "HEDRON_QUEUE_BATCH_SIZE", HEDRON_QUEUE_BATCH_SIZE, 100);
  var totalQueued = 0;
  for (var c = 0; c < jobs.length; c += batchSize) {
    var slice = jobs.slice(c, c + batchSize);
    var messages = [];
    for (var i = 0; i < slice.length; i++) {
      var job = slice[i];
      messages.push({
        body: {
          schema_version: 1,
          upload_id: "hedron:" + job.deckImageUuid,
          cube_id: cubeId,
          deck_image_uuid: job.deckImageUuid,
          draft_id: job.draftId,
          player_id: job.playerId,
          pilot_name: job.pilotName,
          submitted_at: job.recordLogged,
          image_url: hedronImageUrl(job.deckPath),
          image_source: "hedron",
          match_wins: job.wins,
          match_losses: job.losses,
          match_draws: job.draws,
          r2_prefix: hedronR2Prefix(job.deckImageUuid),
        },
        contentType: "json",
      });
    }
...
/**
 * Pull Hedron Network JSON for a cube (CubeCobra id works as cubeId query param).
 * Publishes deck image jobs to Cloudflare Queues in batches; the consumer stages images to R2.
...
async function syncHedronCube(env, cubeId, ctx, depth) {
  ...
  while (pages < maxPagesTick) {
    ...
    if (!page || !Array.isArray(page.drafts)) {
      ...
      throw new Error("invalid Hedron page");
    }
    pagesProcessed++;
    ...
    if (jobsToQueue.length) {
      var queueResult;
      try {
        queueResult = await enqueueHedronDeckJobs(env, id, jobsToQueue);
      } catch (queueErr) {
        console.error("hedron queue publish failed", id, queueErr);
        queueResult = { ok: false, queued: 0, reason: "queue_failed" };
      }
      if (!queueResult.ok) {
        await upsertHedronSyncState(
          env,
          id,
          nextKey,
          0,
      ...
      for (var qi = 0; qi < jobsToQueue.length; qi++) {
    ...
    if (imported >= maxDecksTick) {
      var deckLimitContinuation = false;
      ...
      return {
        ok: true,
        cube_id: id,
        decks_queued: imported,
        pages_processed: pagesProcessed,
        done: false,
        continuation_scheduled: deckLimitContinuation,
      };
    }
...
function buildReprocessUploadId(uploadId) {
  return String(uploadId || "").replace(/\/+$/, "") + ":reprocess:" + crypto.randomUUID();
}
...
async function buildReprocessExtractTask(deck, cubeId, reprocessUploadId, env) {
  ...
  var imageId = await computeImageId(cubeId, pilotName, reprocessUploadId, {
    imageSource: deck.image_source || "",
  });
  var taskBody = {
    upload_id: reprocessUploadId,
    schema_version: 2,
    cube_id: cubeId,
    image_id: imageId,
    oriented_image_r2_key: orientedKey,
    processing_timestamp: reprocessUploadId,
...
async function buildReprocessOrientTask(deck, cubeId, uploadId, reprocessUploadId, sessionUser, env) {
...
async function handleReprocessDeck(deckIdStr, request, env) {
  ...
  if (taskBody.r2_bucket != null) jobTask.r2_bucket = taskBody.r2_bucket;
  if (taskBody.r2_prefix != null) jobTask.r2_prefix = taskBody.r2_prefix;
  else if (taskBody.staging_image_r2_key != null) {
    var stagingPrefix = r2PrefixFromObjectKey(taskBody.staging_image_r2_key);
    if (stagingPrefix) jobTask.r2_prefix = stagingPrefix;
  }
  if (taskBody.image_source != null) jobTask.image_source = taskBody.image_source;
  if (taskBody.match_wins != null) jobTask.match_wins = taskBody.match_wins;
  if (taskBody.match_losses != null) jobTask.match_losses = taskBody.match_losses;
  if (taskBody.match_draws != null) jobTask.match_draws = taskBody.match_draws;
  ...
  if (!enqueueResult.ok) {
    ...
    try {
      await env.cubewizard_db.prepare("DELETE FROM processing_jobs WHERE upload_id = ?").bind(reprocessUploadId).run();
    } catch (cleanupErr) {
...
async function handleAddCube(request, env, ctx) {
  try {
