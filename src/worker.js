...
async function markProcessingJobFailed(db, uploadId, error) {
  await db
    ...
    .bind(String(error || "queue_failed").slice(0, 4000), uploadId)
    .run();
}
...
async function buildReprocessExtractTask(deck, cubeId, uploadId, replaceDeckId, env) {
  ...
  var pilot = String(deck.pilot_name || "");
  var imageSource = deck.image_source || "";
  var imageId = await computeImageId(cubeId, pilot, uploadId, {
...
async function handleReprocessDeck(deckIdStr, request, env) {
  ...
  if (!enqueueResult.ok) {
    ...
    try {
      await markProcessingJobFailed(env.cubewizard_db, uploadId, "queue_failed");
    } catch (jobErr) {
      console.error("processing_jobs fail update failed on reprocess:", jobErr);
    }
