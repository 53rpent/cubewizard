...
function applyStatementToState(state: FakeState, stmt: { sql: string; args: unknown[] }) {
...
function createFakeD1(initial: Partial<FakeState> = {}) {
  ...
  const db = {
    ...
    async batch(stmts: Array<{ sql: string; args: unknown[] }>) {
      return stmts.map((stmt) => applyStatementToState(state, stmt));
    },
  };
  return db;
}
...
describe("executeDeckWritePlan", () => {
  it("continues finalization when an existing same-job deck is already complete", async () => {
  ...
  it("rebuilds missing child rows when a racing insert reports duplicate", async () => {
    const db = createFakeD1({
      deck: { deck_id: 7, cube_id: "cube", image_id: "img", processing_timestamp: "ts1" },
      hasStats: true,
      cards: [],
