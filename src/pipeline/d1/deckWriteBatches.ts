...
import type { D1Statement, DeckCardRow, DeckPayload, DeckWritePlan } from "./types";
 ...
 */
export function deckInsertWasDuplicate(batchAResults: Array<{ meta?: { changes?: number } } | undefined>): boolean {
 ...
 */
export async function buildDeckWritePlan(cubeId: string, deckData: DeckPayload): Promise<DeckWritePlan> {
  ...
  function buildBatchB(deckId: number): D1Statement[] {
    ...
    const totalNotFound = cardsData.total_not_found ?? notFound.length;
    ...
    stmts.push(
      stmt(
        ...
        [deckId, cardsData.total_found ?? 0, totalNotFound, JSON.stringify(processingNotes)],
      ),
    );

    for (const card of cardsData.cards ?? []) {
      stmts.push(deckCardInsertStmt(deckId, card));
    }
