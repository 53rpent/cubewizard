          ...
          return {
            ...
            async run() {
              if (failProcessingJobUpsert && sql.indexOf("INSERT INTO processing_jobs") >= 0) {
                throw new Error("processing_jobs unavailable");
              }
...
function hasProcessingJobInsert(db) {
  return db.calls.some(function (call) {
    return call.type === "run" && call.sql.indexOf("INSERT INTO processing_jobs") >= 0;
  });
}

function hasProcessingJobFailedUpdate(db) {
  return db.calls.some(function (call) {
    return (
      call.type === "run" &&
      call.sql.indexOf("UPDATE processing_jobs SET status = 'failed'") >= 0 &&
      call.args.indexOf("missing_eval_queue") >= 0
    );
  });
}
...
describe("deck reprocess route", () => {
  it("does not delete deck rows when eval queue enqueue cannot be made durable", async () => {
    ...
    expect(hasDeckDeleteBatch(db)).toBe(false);
    expect(hasProcessingJobInsert(db)).toBe(true);
    expect(hasProcessingJobFailedUpdate(db)).toBe(true);
  });

  it("requires durable processing job tracking before queueing reprocess work", async () => {
    var sessionUser = { user_id: 5, username: "owner" };
    var db = createDbMock({
      sessionUser: sessionUser,
  ...
  it("queues reprocess with a fresh replacement identity without deleting the current deck", async () => {
    var sessionUser = { user_id: 5, username: "owner" };
    var db = createDbMock({
      sessionUser: sessionUser,
      deck: {
    ...
    expect(json.upload_id).toMatch(/^reprocess:123:/);
    expect(sentBody.upload_id).toBe(json.upload_id);
    expect(sentBody.processing_timestamp).toBe(json.upload_id);
